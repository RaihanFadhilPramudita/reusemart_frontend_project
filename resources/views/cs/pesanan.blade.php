  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadOrders(search = '') {
      try {
        ...
        if (userStr) {
          ...
          const isCS = user?.jabatan?.NAMA_JABATAN === "Customer Service";
          if (!isCS) {
            console.error('User is not a Customer Service');
            emptyState.classList.remove('hidden');
            loadingState.classList.add('hidden');
        ...
        // Fetch orders
        const statusFilter = currentTab === 'diproses' ? 'diproses' : 'selesai';
        const url = search 
          ? `http://localhost:8000/api/cs/pesanan?status=${statusFilter}&search=${encodeURIComponent(search)}`
          : `http://localhost:8000/api/cs/pesanan?status=${statusFilter}`;
        ...
        console.log('Order data structure:', JSON.stringify(response.data.data?.[0]));
    ...
    window.showOrderDetail = async (orderId) => {
      try {
        ...
        const response = await axios.get(`http://localhost:8000/api/cs/pesanan/${orderId}`, {
          headers: { 
            'Authorization': `Bearer ${token}`,
            'Accept': 'application/json'
          }
    ...
    window.processOrder = async (orderId) => {
      ...
      try {
        ...
        await axios.put(`http://localhost:8000/api/cs/pesanan/${orderId}/selesai`, {}, {
          headers: { 
            'Authorization': `Bearer ${token}`,
            'Accept': 'application/json',
            'Content-Type': 'application/json'
